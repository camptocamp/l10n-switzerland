...
APPENDIX_OPEN = "<Appendix>"
APPENDIX_CLOSE = "</Appendix>"
...
class AccountInvoiceYellowbill(models.AbstractModel):
    ...
    def _get_render_values(
        ...
        amount_sign = -1 if payment_type == "credit" else 1
        # The delivery place is only reported when it differs from the customer.
        delivery = invoice.partner_shipping_id
        if delivery == invoice.partner_id:
        ...
        return {
            ...
            "language": (invoice.partner_id.lang or "en")[:2],
            "order": order,
    ...
    def _get_line_vals_list(self, lines):
        return [
            self._get_line_vals(line, index)
            for index, line in enumerate(lines, start=1)
        ]

    def _get_line_vals(self, line, index):
        return {
            "line": line,
            "index": index,
    ...
    def _get_line_tax_amount(self, line, tax):
        ...
        :param line: the ``account.move.line`` to compute.
        :param tax: the ``account.tax`` to compute on that line.
        """
        if not tax:
            return 0.0
    ...
    def _validate_payload(self, payload):
        """Check a payload against the YellowBill schema."""
        schema = etree.XMLSchema(file=file_path(XML_SCHEMA))
        parser = etree.XMLParser(schema=schema)
        try:
            etree.fromstring(payload.encode("utf-8"), parser)
        except etree.XMLSyntaxError as ex:
            raise UserError(str(ex.error_log)) from ex

    def _strip_appendix(self, payload):
        """Remove the embedded PDF from a payload.

        The appendix is by far the biggest part of the payload, so it is
        stripped before keeping a copy of what was sent.
        """
        start = payload.find(APPENDIX_OPEN)
        if start < 0:
            return payload
        end = payload.find(APPENDIX_CLOSE, start)
        return payload[0:start] + payload[end + len(APPENDIX_CLOSE) :]
