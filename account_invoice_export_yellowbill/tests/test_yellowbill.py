...
class TestYellowbill(CommonCase):
    ...
    def test_strip_appendix(self):
        """The embedded PDF can be stripped, to keep a small copy of the payload."""
        payload = self.builder._export_invoice(
            self.invoice,
            biller_id=BILLER_ID,
            ebill_account_id=EBILL_ACCOUNT_ID,
        ...
        self.assertIn("cGRmZGF0YQ==", payload)
        self.assertNotIn("cGRmZGF0YQ==", self.builder._strip_appendix(payload))
