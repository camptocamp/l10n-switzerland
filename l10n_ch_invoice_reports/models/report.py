...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf(self, res_ids=None, data=None):
        ...
        inv_report = self.env['ir.actions.report'].browse(int(self.env['ir.config_parameter'].sudo().get_param('invoice_report_id'))) or self._get_report_from_name("account.report_invoice")
        qr_report = self.env['ir.actions.report'].browse(int(self.env['ir.config_parameter'].sudo().get_param('qr_report_id'))) or self._get_report_from_name("l10n_ch.qr_report_main")
        io_list = []
